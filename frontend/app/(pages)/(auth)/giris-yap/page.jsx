...
import api from "@/app/utilis/api"; // axios instance
...
const LoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // login isteği
      await api.post("/auth/login", { ...formData, rememberMe: isRemember });
      toast.success("Giriş Yapıldı");

      // login sonrası kullanıcı bilgisi al
      ...
      <article className="lg:w-1/2 w-full mt-20 lg:mt-2 flex items-center justify-center">
        <div className="md:w-1/2 w-3/4 h-[60vh] -mt-20 flex flex-col items-center gap-2 p-5">
          ...
          >
            ...
            <button
              type="submit"
              className="w-1/2 py-2 px-4 mt-4 rounded-lg text-white font-semibold cursor-pointer 
